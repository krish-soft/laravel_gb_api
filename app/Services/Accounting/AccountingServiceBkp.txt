...
class AccountingService
{
    ...
    public function markSettled(AccountLedger $ledger): void
    {
        ...
        DB::transaction(function () use ($ledger) {
            ...
            if (!$ledger->is_tax && $ledger->debit > 0) {
                $account->update([
                    'available_balance' => DB::raw("available_balance - {$ledger->debit}"),
                ]);
            }
    ...
    {
        $updates = [
            'total_credit' => DB::raw("total_credit + {$ledger->credit}"),
            'total_debit'  => DB::raw("total_debit + {$ledger->debit}"),
        ];
        ...
        if (!$ledger->is_tax) {
            ...
            if ($ledger->status === LedgerStatusEnum::PENDING->value) {
                $updates['hold_balance'] = DB::raw("hold_balance + {$net}");
            }

            if ($ledger->status === LedgerStatusEnum::AVAILABLE->value) {
                $updates['available_balance'] = DB::raw("available_balance + {$net}");
            }
        }
