        ...
        try {
            ...
            DB::transaction(function () use ($listingService) {
                ...
                foreach ($listings as $listing) {
                    ...
                    /* ---------------------------------------------
                     | CREATE MARKET ITEMS PER PACKAGE (NO GROUPING)
                     ---------------------------------------------*/
                    foreach ($listing->listingItems as $item) {
                        ...
                        foreach ($item->listingPackages as $pkg) {
                            ...
                            for ($i = 0; $i < $remain; $i++) {
                                ...
                                /* ---------------------------------------------
                                 | CREATE SHIPMENT DIRECTLY FROM PACKAGE
                                 ---------------------------------------------*/
                                ShipmentPackage::create([
                                    ...
                                    'buyer_id' => null, //$marketOrder->buyer_id,
                                    'seller_id' => $item->productListing->seller_id,

                                    'pickup_fulfillment_location_id' => $marketItem->pickup_fulfillment_location_id,
                                    ...
                                    'qty' => 1,
                                    'pack_size' => $marketItem->pack_size,
                                    'pack_price' => $marketItem->pack_price,
                                    'pack_unit' => $marketItem->pack_unit,
                                    'pack_type_unit' => $marketItem->pack_type_unit,

                                    'package_number' => ShipmentPackage::generatePackageNumber(null, $marketId),
                                    'status' => ShipmentStatusEnum::PENDING->value,

                                    'pickup_depot_id' => $item?->pickup_depot?->depot_id ?? $marketOrder->depot_id, // to prevent transfer
                                    'shipping_depot_id' => $marketOrder->depot_id, // to prevent transfer

                                    'is_buyer_pickup' => $marketOrder->is_buyer_pickup, // From order directly
